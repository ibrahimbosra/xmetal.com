(function (global) {
  ...
  function toPrimaryAmount(value, rate, currencyCode) {
    var amount = toNumber(value);
    if (amount === null) return 0;
    var targetRate = toNumber(rate) || 1;
    var normalizedCode = String(currencyCode || '').trim().toLowerCase();
    if (targetRate > 0 && normalizedCode === '') {
      return amount / targetRate;
    }
    return amount;
  }

  function toSecondaryAmount(value, rate, currencyCode) {
    var amount = toNumber(value);
    if (amount === null) return 0;
    var targetRate = toNumber(rate) || 1;
    var normalizedCode = String(currencyCode || '').trim().toLowerCase();
    if (targetRate > 0 && normalizedCode === 'primary') {
      return amount * targetRate;
    }
    return amount;
  }
  ...
  var api = {
    ...
    toPrimaryAmount: toPrimaryAmount,
