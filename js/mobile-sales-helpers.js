(function (global) {
  ...
  function getProductCurrencyDisplay(primaryAmount, settings) {
    var source = settings || {};
    var code = source.baseCurrency === '' ? '' : 'primary';
    var rate = Number(source.rate) || 1;
    var primaryValue = Number(primaryAmount) || 0;
    var Value = primaryValue * rate;
    return {
      primaryCode: code,
      primaryValue: code === '' ? Value : primaryValue,
      primarySymbol: code === '' ? (source.CurrencySymbol || '') : '$',
      Value: code === '' ? primaryValue : Value,
      Symbol: code === '' ? '$' : (source.CurrencySymbol || ''),
      showSecondary: source.showProductPrices === true
    };
  }
  ...
  function getSaleReferenceAmount(sale) {
    var saleRecord = sale || {};
    if (saleRecord.baseCurrency === 'primary' && saleRecord.baseAmount != null && Number.isFinite(Number(saleRecord.baseAmount))) {
      return Number(saleRecord.baseAmount);
    }
    var candidates = [
  ...
  function getCustomerDebtBalance(customerId, sales, debtOperations) {
    ...
    var debtTotal = activeSales.reduce(function (sum, sale) { return sum + getSaleNetAmount(sale); }, 0);
    debtTotal += operations.filter(function (entry) {
      if (!entry || String(entry.customerId || '') !== customerKey || String(entry.status || '').toLowerCase() === 'cancelled' || String(entry.status || '').toLowerCase() === 'deleted' || String(entry.status || '').toLowerCase() === 'returned') return false;
  ...
  function getReturnSaleSuggestedCashAmount(sale, sales, debtOperations) {
    var saleRecord = sale || {};
    var saleAmount = Number(saleRecord.displayTotalAmount || saleRecord.rawTotalAmount || saleRecord.totalAmount || saleRecord.amount || 0) || 0;
  ...
  function reassignPaymentsForCancelledSale(customerId, cancelledSaleId, sales, debtOperations) {
    ...
    activeSales.forEach(function (sale) {
      ...
      var totalAmount = Number(sale.totalAmount || sale.amount || 0) || 0;
  ...
  var helpers = {
    ...
    getEmployeeDayId: function (employeeId, dateKey) { return 'employee_day_' + String(employeeId) + '_' + String(dateKey); },
    getEmployeeWageForDay: function (employee, dateKey) {
      ...
      return Number(applicable ? applicable.dailyWage : employee && employee.dailyWage) || 0;
    },
    calculateEmployeeBalance: function (operations) {
