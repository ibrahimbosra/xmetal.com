(function (global) {
  ...
  function groupDebtOperationsByCycles(operations) {
    var source = Array.isArray(operations) ? operations.filter(function (entry) {
      ...
      if (type !== 'debt' && type !== 'payment' && type !== 'settle' && type !== 'return') return false;
      return String(entry.status || '').trim().toLowerCase() !== 'cancelled';
    }) : [];

    var ordered = source.slice().sort(function (a, b) {
      var dateDifference = getDebtCycleDateValue(a) - getDebtCycleDateValue(b);
      if (dateDifference !== 0) return dateDifference;
      var typeOrder = { debt: 1, payment: 2, settle: 3 };
      return (typeOrder[String(a.type || '').trim().toLowerCase()] || 4) - (typeOrder[String(b.type || '').trim().toLowerCase()] || 4);
    });
    ...
    return cycles.reverse().map(function (cycle) {
      ...
      (cycle.entries || []).forEach(function (entry) {
        ...
        else if (type === 'payment' || type === 'settle' || type === 'return') balance -= amount;
      });
      return {
  ...
  function normalizeSaleForCancellation(sale) {
    ...
    if (!Number.isFinite(quantity) || quantity <= 0) {
      var totalAmount = getSaleReferenceAmount(record);
  ...
  function convertSecondaryToPrimary(value, rate) {
    var numericValue = Number(value) || 0;
    var rate = Number(rate) || 1;
    if (!Number.isFinite(numericValue)) return 0;
    if (!Number.isFinite(rate) || rate <= 0) return numericValue;
    return numericValue / rate;
  }

  function convertPrimaryToSecondary(value, rate) {
    var numericValue = Number(value) || 0;
    var rate = Number(rate) || 1;
    if (!Number.isFinite(numericValue)) return 0;
    if (!Number.isFinite(rate) || rate <= 0) return numericValue;
  ...
  function getActiveCurrencyDisplay(value, sourceCurrency, settings) {
    var numericValue = Number(value) || 0;
    var sourceSettings = settings || {};
    var finalSymbol = (sourceSettings && sourceSettings.CurrencySymbol) || '$';
    var activeCode = 'primary';
    var primaryValue = Number(sourceCurrency === '' && Number(sourceSettings.rate) ? numericValue / Number(sourceSettings.rate) : numericValue) || 0;
    if (Number.isFinite(primaryValue) && Math.abs(primaryValue - Math.round(primaryValue)) < 1e-9) {
    ...
    return {
      ...
      symbol: '$',
      code: activeCode,
      label: moneyString(primaryValue) + ' $'
    };
  }
  ...
  function getProductCurrencyDisplay(primaryAmount, settings) {
    var primaryValue = Number(primaryAmount) || 0;
    return {
      primaryCode: 'primary',
  ...
  function getSaleReferenceAmount(sale) {
    var saleRecord = sale || {};
    var candidateModes = ['saleMode', 'currency', 'baseCurrency', 'displayCurrency'];
    var saleMode = 'primary';
    for (var i = 0; i < candidateModes.length; i += 1) {
      var candidateKey = candidateModes[i];
      if (Object.prototype.hasOwnProperty.call(saleRecord, candidateKey) && saleRecord[candidateKey] != null) {
        saleMode = String(saleRecord[candidateKey]).trim().toLowerCase();
        break;
      }
    }
    if (saleMode !== 'primary' && saleMode !== '') saleMode = 'primary';

    var canonicalCandidates = [
      saleRecord.baseAmount,
      saleRecord.totalAmount,
      saleRecord.amount,
      saleRecord.originalAmount,
      saleRecord.netAmount
    ];
    var selectedAmount = null;
    for (var i = 0; i < canonicalCandidates.length; i += 1) {
      var candidateValue = canonicalCandidates[i];
      if (candidateValue == null) continue;
      var numeric = Number(candidateValue);
      if (Number.isFinite(numeric)) {
        selectedAmount = numeric;
        break;
      }
    }

    if (selectedAmount == null || !Number.isFinite(selectedAmount)) return 0;

    if (saleMode === '') {
      var historicalRate = Number(saleRecord.rateAtTime != null ? saleRecord.rateAtTime : 0) || 0;
      if (historicalRate > 0) {
        return selectedAmount / historicalRate;
      }
      return 0;
    }
  ...
  function getDisplayMoney(value, sourceCurrency, settings) {
    var numericValue = Number(value) || 0;
    if (Number.isFinite(numericValue) && Math.abs(numericValue - Math.round(numericValue)) < 1e-9) {
  ...
  function getHistoricalOperationDisplay(operation, settings) {
    var op = operation || {};
    var sourceSettings = settings || {};
    var activeCode = sourceSettings.baseCurrency === '' ? '' : 'primary';
    var numericValue = Number(op.amount) || 0;
    var sourceCurrency = String(op.currency || 'primary').trim().toLowerCase();
    if (sourceCurrency !== 'primary' && sourceCurrency !== '') sourceCurrency = 'primary';
    var historicalRate = Number(op.rateAtTime != null ? op.rateAtTime : (op.rateAtTime != null ? op.rateAtTime : sourceSettings.rate)) || Number(sourceSettings.rate) || 1;
    var convertedValue = numericValue;
    if (sourceCurrency === 'primary' && activeCode === '') convertedValue = numericValue * historicalRate;
    else if (sourceCurrency === '' && activeCode === 'primary') convertedValue = numericValue / historicalRate;
    ...
    return {
      ...
      symbol: activeCode === '' ? (sourceSettings.CurrencySymbol || '') : '$',
      code: activeCode
    };
  }
  ...
  function getSaleNetAmount(sale) {
    ...
    var refundCurrency = String(saleRecord.refundCurrency || '').trim().toLowerCase();
    var saleMode = String(saleRecord.baseCurrency || saleRecord.saleMode || saleRecord.displayCurrency || saleRecord.currency || 'primary').trim().toLowerCase();
    if (saleMode !== 'primary' && saleMode !== '') saleMode = 'primary';
    var rate = Number(saleRecord.rateAtTime != null ? saleRecord.rateAtTime : 1) || 1;
    if (refundAmount > 0 && refundCurrency !== saleMode) {
      if (refundCurrency === '' && saleMode === 'primary') {
        refundAmount = refundAmount / rate;
      } else if (refundCurrency === 'primary' && saleMode === '') {
        refundAmount = refundAmount * rate;
      } else if (refundCurrency === 'primary' && saleMode === 'primary') {
        refundAmount = refundAmount;
      }
    }
    if (refundAmount > 0) return Math.max(0, amount - refundAmount);
    return amount;
  }
  ...
  function getSalePaidAmount(sale, debtOperations) {
    ...
    list.forEach(function (entry) {
      ...
      if (entryStatus === 'cancelled' || entryStatus === 'deleted') return;
      ...
      if (allocations.length) {
        allocations.forEach(function (allocation) {
          ...
          var allocationStatus = String(allocation.status || '').trim().toLowerCase();
          if (allocationStatus === 'cancelled' || allocationStatus === 'deleted') return;
  ...
  function reconcileCreditSaleDebtAfterEdit(sale, debtOperations) {
    ...
    var debtAmount = getSaleReferenceAmount(saleRecord);
  ...
  function getCustomerSalePaymentAllocations(sales, debtOperations) {
    ...
    var paymentEntries = Array.isArray(debtOperations) ? debtOperations.filter(function (entry) {
      ...
      return entry && (entry.type === 'payment' || entry.type === 'settle') && status !== 'cancelled' && status !== 'deleted' && String(entry.customerId || '').trim();
    }) : [];
    var groupedSales = {};
  ...
  function getCustomerDebtBalanceLegacy(customerId, sales, debtOperations) {
    ...
    var directDebtEntries = Array.isArray(debtOperations) ? debtOperations.filter(function (entry) {
      ...
      return type === 'debt' && !saleRef;
    }) : [];

    var directDebtTotal = directDebtEntries.reduce(function (sum, entry) {
      return sum + (Number(entry.amount) || 0);
    }, 0);

    var directDebtPaid = 0;
    ...
    directEntries.forEach(function (entry) {
      ...
      if (allocations.length) {
        allocations.forEach(function (allocation) {
          if (!allocation) return;
          var debtLink = normalizeIdValue(allocation.debtOperationId || allocation.linkedDebtOperationId || allocation.relatedDebtOperationId || '');
          if (!debtLink) return;
          var debtEntry = directDebtEntries.find(function (item) {
            var id = normalizeIdValue(item.id || item.debtOperationId || item.operationId || '');
            return String(id) === String(debtLink);
          });
          if (debtEntry) {
            directDebtPaid += Number(allocation.amount) || 0;
          }
        });
        return;
      }
      if ((type === 'payment' || type === 'settle') && customerSales.length === 0) {
        directDebtPaid += Number(entry.amount) || 0;
      }
    });

    return Math.max(0, totalDebt + directDebtTotal - directDebtPaid);
  }

  function getCustomerDebtBalance(customerId, sales, debtOperations) {
    var customerKey = String(customerId || '');
    function getPrimaryAmount(record) {
      var source = record || {};
      var baseAmount = Number(source.baseAmount);
      var baseCurrency = String(source.baseCurrency || '').trim().toLowerCase();
      var currency = String(source.currency || source.saleMode || source.baseCurrency || 'primary').trim().toLowerCase();
      var rate = Number(source.rateAtTime != null ? source.rateAtTime : source.rateAtTime) || 0;
      var hasPrimaryBaseAmount = Number.isFinite(baseAmount) && baseCurrency === 'primary';
      var amount = hasPrimaryBaseAmount ? baseAmount : Number(source.amount != null ? source.amount : (source.totalAmount != null ? source.totalAmount : source.displayTotalAmount)) || 0;
      var primaryAmount = hasPrimaryBaseAmount || currency !== '' ? amount : (rate > 0 ? amount / rate : 0);
      var refundAmount = Number(source.refundAmount || source.refundedAmount || source.returnedAmount || 0) || 0;
      var refundCurrency = String(source.refundCurrency || currency).trim().toLowerCase();
      if (refundAmount > 0 && refundCurrency === '') refundAmount = rate > 0 ? refundAmount / rate : 0;
      return Math.max(0, primaryAmount - refundAmount);
    }
    function getPaymentPrimaryAmount(operation, allocation) {
      var linkedSale = allocation && allocation.saleId ? (Array.isArray(sales) ? sales.find(function (sale) {
        return sale && String(sale.saleId || sale.id) === String(allocation.saleId);
      }) : null) : null;
      var source = linkedSale || operation || {};
      var currency = String(source.currency || source.saleMode || source.baseCurrency || 'primary').trim().toLowerCase();
      var amount = Number(allocation ? allocation.amount : operation && operation.amount) || 0;
      if (currency !== '') return amount;
      var rate = Number(source.rateAtTime != null ? source.rateAtTime : source.rateAtTime) || 0;
      return rate > 0 ? amount / rate : 0;
    }
    var activeSales = Array.isArray(sales) ? sales.filter(function (sale) {
      var saleStatus = String(sale && sale.status || '').toLowerCase();
      return sale && String(sale.customerId || '') === customerKey && !sale.cancelled && !sale.isCancelled && saleStatus !== 'cancelled' && saleStatus !== 'returned' && saleStatus !== 'refunded' && isOriginalCreditSale(sale);
    }) : [];
    var saleKeys = new Set(activeSales.map(function (sale) { return normalizeIdValue(sale.saleId || sale.id); }));
    var operations = Array.isArray(debtOperations) ? debtOperations : [];
    var debtTotal = activeSales.reduce(function (sum, sale) { return sum + getPrimaryAmount(sale); }, 0);
    debtTotal += operations.filter(function (entry) {
      if (!entry || String(entry.customerId || '') !== customerKey || String(entry.status || '').toLowerCase() === 'cancelled' || String(entry.status || '').toLowerCase() === 'deleted') return false;
      var type = String(entry.type || '').toLowerCase();
      var linkedSale = normalizeIdValue(entry.saleId || entry.relatedSaleId || entry.linkedSaleId || entry.debtSaleId || entry.objectId || '');
      return type === 'debt' && (!linkedSale || !saleKeys.has(linkedSale));
    }).reduce(function (sum, entry) { return sum + getPrimaryAmount(entry); }, 0);
    var payments = operations.filter(function (entry) {
      if (!entry || String(entry.customerId || '') !== customerKey || String(entry.status || '').toLowerCase() === 'cancelled' || String(entry.status || '').toLowerCase() === 'deleted') return false;
      var type = String(entry.type || '').toLowerCase();
      return type === 'payment' || type === 'settle';
    }).reduce(function (sum, entry) {
      var allocations = Array.isArray(entry.allocations) ? entry.allocations : [];
      if (allocations.length) return sum + allocations.reduce(function (allocationSum, allocation) {
        return allocationSum + getPaymentPrimaryAmount(entry, allocation);
      }, 0);
      return sum + getPaymentPrimaryAmount(entry);
    }, 0);
    var returns = operations.filter(function (entry) {
      return entry && String(entry.customerId || '') === customerKey && String(entry.type || '').toLowerCase() === 'return' && String(entry.status || '').toLowerCase() !== 'cancelled' && String(entry.status || '').toLowerCase() !== 'deleted';
    }).reduce(function (sum, entry) { return sum + getPrimaryAmount(entry); }, 0);
    return Math.max(0, debtTotal - payments - returns);
  }

  function getReturnSaleSuggestedCashAmount(sale, sales, debtOperations) {
    var saleRecord = sale || {};
    var saleAmount = getSaleReferenceAmount(saleRecord);
    if (saleAmount <= 0) return 0;
    var customerId = normalizeIdValue(saleRecord.customerId);
    if (!customerId) return 0;
    var saleTimestamp = Number(saleRecord.timestamp) || 0;
    var hasLaterPayment = Array.isArray(debtOperations) ? debtOperations.some(function (entry) {
      if (!entry || String(entry.customerId || '') !== customerId) return false;
      var type = String(entry.type || '').trim().toLowerCase();
      if (type !== 'payment' && type !== 'settle') return false;
      ...
      if (saleTimestamp > 0 && timestamp <= saleTimestamp) return false;
      return true;
    }) : false;
    return hasLaterPayment ? saleAmount : 0;
  }
  ...
  function getDebtOperationPaymentProgress(entry, paymentEntries) {
    ...
    var customerId = String(entry.customerId || '').trim();
    ...
    payments.forEach(function (payment) {
      ...
      if (allocations.length) {
        allocations.forEach(function (allocation) {
          ...
          var allocationStatus = String(allocation.status || payment.status || 'active').trim().toLowerCase();
          if (allocationStatus === 'refunded' || allocationStatus === 'returned' || allocationStatus === 'cancelled') return;
          if (allocationStatus === 'partially_refunded' || allocationStatus === 'partially_returned') {
            var refundedShare = Number(allocation.refundedAmount || allocation.refundAmount || 0) || 0;
            if (refundedShare > 0) {
              var activeAmount = Math.max(0, (Number(allocation.amount) || 0) - refundedShare);
              if ((debtId && candidateDebtId === debtId) || (saleId && candidateSaleId === saleId)) {
      ...
      if (saleId && String(payment.saleId || payment.relatedSaleId || payment.linkedSaleId || '') === saleId) {
        paidAmount += Number(payment.amount) || 0;
        return;
      }

      if (!customerId || String(payment.customerId || '').trim() !== String(customerId)) return;
      var paymentAmount = Number(payment.amount) || 0;
      if (paymentAmount <= 0) return;

      var debtEntries = payments.filter(function (candidate) {
        return candidate && String(candidate.customerId || '').trim() === String(customerId) && (String(candidate.type || '').trim().toLowerCase() === 'debt');
      }).sort(function (a, b) { return (Number(a.timestamp) || 0) - (Number(b.timestamp) || 0); });
      var remaining = paymentAmount;
      var paidByDebt = {};
      debtEntries.forEach(function (debtEntry) {
        if (remaining <= 0) return;
        var debtEntryId = String(debtEntry.id || debtEntry.debtOperationId || debtEntry.operationId || '');
        if (!debtEntryId) return;
        var debtEntryAmount = Number(debtEntry.amount) || 0;
        var alreadyPaid = Number(paidByDebt[debtEntryId]) || 0;
        var available = Math.max(0, debtEntryAmount - alreadyPaid);
        if (available <= 0) return;
        var applied = Math.min(remaining, available);
        if (String(debtEntryId) === String(debtId)) {
          paidAmount += applied;
        }
        paidByDebt[debtEntryId] = alreadyPaid + applied;
        remaining -= applied;
      });
    });
  ...
  function getEmployeeDayId(employeeId, dateKey) {
    return 'employee_day_' + String(employeeId) + '_' + String(dateKey);
  }

  function getEmployeeWageForDay(employee, dateKey) {
    ...
    var applicable = history.find(function (entry) { return entry && String(entry.effectiveFrom || '') <= String(dateKey); });
    ...
    if (String(source.dailyWageCurrency || 'primary').trim().toLowerCase() === '' && source.dailyWagePrimary != null) {
      primaryValue = Number(source.dailyWagePrimary) / (Number(source.rate) || 1);
    }
    return primaryValue;
  }

  function getEmployeeWageForCurrency(employee, currencyCode, settings) {
    var source = employee || {};
    var rate = Number((settings && settings.rate) || source.rate || 1) || 1;
    var primaryValue = Number(source.dailyWage != null ? source.dailyWage : (source.dailyWagePrimary != null ? source.dailyWagePrimary : 0)) || 0;
    var sourceCurrency = String(source.dailyWageCurrency || 'primary').trim().toLowerCase();
    if (sourceCurrency === '' && source.dailyWagePrimary != null) {
      primaryValue = Number(source.dailyWagePrimary) / rate;
    } else if (sourceCurrency === 'primary' && source.dailyWagePrimary != null && source.dailyWage == null && rate > 0) {
      primaryValue = Number(source.dailyWagePrimary) / rate;
    }
    var code = String(currencyCode || source.dailyWageCurrency || 'primary').trim().toLowerCase();
    if (code !== 'primary' && code !== '') code = 'primary';
    return code === '' ? primaryValue * rate : primaryValue;
  }

  function getEmployeeWageDisplay(employee, currencyCode, settings) {
    var sourceSettings = settings || {};
    var code = String(currencyCode || sourceSettings.baseCurrency || (employee && employee.dailyWageCurrency) || 'primary').trim().toLowerCase();
    if (code !== 'primary' && code !== '') code = 'primary';
    var primaryValue = Number(employee && employee.dailyWage != null ? employee.dailyWage : (employee && employee.dailyWagePrimary != null ? employee.dailyWagePrimary : 0)) || 0;
    var rate = Number(sourceSettings.rate || (employee && employee.rate) || 1) || 1;
    var sourceCurrency = String(employee && employee.dailyWageCurrency || 'primary').trim().toLowerCase();
    if (sourceCurrency === '' && employee && employee.dailyWagePrimary != null) {
      primaryValue = Number(employee.dailyWagePrimary) / rate;
    } else if (sourceCurrency === 'primary' && employee && employee.dailyWagePrimary != null && employee.dailyWage == null && rate > 0) {
      primaryValue = Number(employee.dailyWagePrimary) / rate;
    }
    var value = code === '' ? primaryValue * rate : primaryValue;
    return {
      value: value,
      symbol: code === '' ? (sourceSettings.CurrencySymbol || '') : '$',
      code: code
    };
  }

  function normalizeEmployeeOperationAmount(entry) {
    if (!entry || typeof entry !== 'object') return 0;
    var amount = Number(entry.amount) || 0;
  ...
  function calculateEmployeeBalance(operations) {
    return (Array.isArray(operations) ? operations : []).filter(function (entry) {
      if (!entry || typeof entry !== 'object') return false;
      var status = String(entry.status || '').trim().toLowerCase();
      var cancelled = entry.cancelled === true || entry.isCancelled === true || status === 'cancelled' || status === 'deleted' || status === 'returned';
      return !cancelled;
    }).reduce(function (balance, entry) {
      var amount = normalizeEmployeeOperationAmount(entry);
      var isCreditLike = entry.type === 'wage' || entry.type === 'addition' || entry.type === 'employee_withdrawal_return';
      return balance + (isCreditLike ? amount : -amount);
    }, 0);
  }

  function getEmployeeProductValue(operations) {
    if (!Array.isArray(operations)) return 0;
    return operations.filter(function (entry) {
      if (!entry || typeof entry !== 'object') return false;
      var status = String(entry.status || '').trim().toLowerCase();
      var cancelled = entry.cancelled === true || entry.isCancelled === true || status === 'cancelled' || status === 'deleted' || status === 'returned';
      return !cancelled && (entry.type === 'productPurchase' || entry.type === 'employee_withdrawal' || entry.type === 'employee_withdrawal_return');
    }).reduce(function (sum, entry) {
      var amount = Number(entry.amount) || 0;
      if (entry.type === 'employee_withdrawal_return') return sum + amount;
      if (entry.type === 'employee_withdrawal') return sum + amount;
      if (entry.type === 'productPurchase') return sum + amount;
      return sum;
    }, 0);
  }

  function getEmployeeCurrentBalance(employee, operations) {
    var employeeRecord = employee || {};
    var operationalBalance = calculateEmployeeBalance(operations);
    if (Array.isArray(operations) && operations.length > 0) return operationalBalance;
    var storedBalance = Number(employeeRecord.currentBalance != null ? employeeRecord.currentBalance : NaN);
    if (Number.isFinite(storedBalance)) return storedBalance;
    return operationalBalance;
  }

  function getEmployeeBalanceColorClass(value) {
    var numericValue = Number(value) || 0;
    if (numericValue < 0) return 'negative';
    return 'positive';
  }

  function isEmployeeOperationActive(operation) {
  ...
  function isEmployeeOperationEditable(operation) {
    return false;
  }

  function getEmployeeWithdrawalEditImpact(previousOperation, nextAmount) {
    var previousAmount = Number(previousOperation && previousOperation.amount) || 0;
    var nextValue = Number(nextAmount) || 0;
    var delta = nextValue - previousAmount;
    return {
      balanceDelta: delta,
      amountDelta: delta,
  ...
  function getEmployeeGoodsEditImpact(previousOperation, nextQuantity, nextUnitPrice) {
    var previousQuantity = Number(previousOperation && previousOperation.quantity) || 0;
    var nextQty = Number(nextQuantity) || 0;
    var nextPrice = Number(nextUnitPrice) || 0;
    var previousPrice = Number(previousOperation && previousOperation.unitPrice) || 0;
    var previousAmount = Number(previousOperation && previousOperation.amount) || 0;
    var newAmount = nextQty * nextPrice;
    return {
      quantityDelta: nextQty - previousQuantity,
  ...
  function reassignPaymentsForCancelledSale(customerId, cancelledSaleId, sales, debtOperations) {
    ...
    activeSales.forEach(function (sale) {
      ...
      var totalAmount = getSaleReferenceAmount(sale) || 0;
  ...
  var helpers = {
    ...
    getProductCurrencyDisplay: getProductCurrencyDisplay,
    getActiveCurrencyDisplay: getActiveCurrencyDisplay,
    getDisplayMoney: getDisplayMoney,
    getHistoricalOperationDisplay: getHistoricalOperationDisplay,
    ...
    getReturnSaleSuggestedCashAmount: getReturnSaleSuggestedCashAmount,
    ...
    getEmployeeDayId: getEmployeeDayId,
    getEmployeeWageForDay: getEmployeeWageForDay,
    getEmployeeWageForCurrency: getEmployeeWageForCurrency,
    getEmployeeWageDisplay: getEmployeeWageDisplay,
    calculateEmployeeBalance: calculateEmployeeBalance,
    getEmployeeCurrentBalance: getEmployeeCurrentBalance,
    getEmployeeProductValue: getEmployeeProductValue,
    getEmployeeBalanceColorClass: getEmployeeBalanceColorClass,
    isEmployeeOperationActive: isEmployeeOperationActive,
    isEmployeeOperationEditable: isEmployeeOperationEditable,
    getEmployeeWithdrawalEditImpact: getEmployeeWithdrawalEditImpact,
    getEmployeeGoodsEditImpact: getEmployeeGoodsEditImpact
  };
